...
class RouterAgent:
    ...
    def _load_registry(self) -> Dict[str, Any]:
        ...
        try:
            # Path calculation:
            # __file__ = C:\...\lc-agentic-processor\src\agents\router_agent.py
            # parent = C:\...\lc-agentic-processor\src\agents
            # parent.parent = C:\...\lc-agentic-processor\src
            # parent.parent.parent = C:\...\lc-agentic-processor
            # Then add 'config/document_registry.yaml'
    ...
    def _initialize_extractors(self):
        """Initialize all extractors that have extract=true."""
        
        for doc in self.registry['documents']:
            if doc.get('extract', False):
                doc_type = doc['type']
                extractor_class_name = doc['extractor_class']
                
                try:
                    # Dynamically import extractor
                    module_name = f"src.agents.extractors.{doc_type}_extractor"
                    ...
                    self.extractors[doc_type] = extractor_class(self.llm)
                    
                    logger.info(f"  ✓ Loaded {doc_type} extractor")
    ...
    def get_extractor(self, doc_type: str):
        ...
        Returns:
            BaseExtractor: Specific extractor instance or None
        """
    ...
    def route_and_extract(self, doc_type: str, text: str) -> Optional[Dict[str, Any]]:
        ...
        # Step 4: Extract
        logger.info(f"  ✓ Using {doc_type} extractor")
